...
arguments: [service_name, condition]
context: fork
...
# New Alert — `$service_name`: $condition

Generate an alert rule for: **$condition**, for service `$service_name`.

## Steps

1. Preflight — confirm the metric(s) needed for "$condition" exist for `$service_name`.
