---
name: k8s
description: Run kubectl actions (get, describe, logs, restart, scale, exec, etc.) against the Housi Kubernetes environments — sandbox, homolog, and production — always with explicit --context and --namespace, discovering service/deployment names at runtime instead of assuming them. Production and homolog share one GKE cluster (gke-prd-housi-01), distinguished only by namespace; a separate PreToolUse hook (housi-k8s-guard.sh) asks for confirmation before any state-changing command reaches that cluster — that prompt is expected, not an error. Use whenever the user wants to inspect, restart, scale, or exec into a Housi service running in Kubernetes. For port-forward/tunneling specifically, use the `k8s-pf` skill instead — it keeps a pre-mapped registry and supervises multiple simultaneous tunnels.
...
For port-forward specifically, see rule 5 below — it's delegated to the `k8s-pf`
skill rather than built here.
...
4. **Ports still vary by service** — `severino-v2/deploy/_config.yml` shows `servicePort`
   varying by stage for that service specifically, but this is not universal: verified
   live, 42/43 sandbox and 39/42 production app services expose port 80, and a handful
   (e.g. `yoda`) don't. For any one-off `get`/`describe`/`logs` action, still confirm the
   real port before assuming 80:
   ```
   kubectl --context <ctx> -n <ns> get svc <service>-service -o jsonpath='{.spec.ports[0].port}'
   ```

5. **Port-forward is handled by the `k8s-pf` skill, not here.** Use `/k8s-pf <service>
   --env <env>` instead of building a raw `kubectl port-forward` yourself — it keeps a
   pre-mapped registry of services (app, StatefulSet-backed databases, and observability
   targets like the OTel collector) with real ports and resource names, supervises
   multiple simultaneous tunnels with automatic reconnect/backoff, and self-refreshes
   when a target isn't mapped yet. See `skills/k8s-pf/SKILL.md`.
...
3. If the action needs a resource name you haven't confirmed this session (deployment,
   ...
   convention.
4. Build and run the full kubectl command with explicit `--context`/`--namespace`. If
   the action is `port-forward`, stop here and hand off to the `k8s-pf` skill instead
   (rule 5) — don't build the port-forward command in this skill.
