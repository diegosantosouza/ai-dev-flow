...
The structural patterns (§1–§11, §19–§25 — staging, rhythm by rule, formatting by rule,
chatbot leftovers) transfer directly to PT-BR prose: "não é apenas X, é Y", listas com
travessão em todo item, ou "Isso não significa apenas X. Significa Y." are the same tells.
The word list in §12 ("Overused AI words") is English-only — for PT-BR, watch instead for
the equivalent inflation words that recur in Claude/GPT output: *fundamental*, *crucial*,
*robusto*, *holístico*, *mergulhar/aprofundar*, *panorama*, *tapeçaria*, *testemunho*,
*meticuloso*, *promover* (verbo figurado). Apply the same rule: a formal PT-BR word outside
this list is not a tell by itself.
